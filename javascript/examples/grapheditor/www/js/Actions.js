...
function getCellById(root, pid) {
    if (root == null || pid == null) {
        return null;
    }
    if (root.getId() == pid) {
        return root;
...
function collectDeviceId(cell, deviceIdSet) {
    if (cell.isVertex()) {
        var deviceId = cell.getdeviceid();
        if (deviceId == null) {
            //console.log("1deviceId is  "+cell.getdeviceid());
            //console.log("2deviceId is  "+cell.getmultiValue());
            return;
        }
    ...
    for (var i = 0; i < cell.getChildCount(); i++) {
        var child = cell.getChildAt(i);
        collectDeviceId(child, deviceIdSet);
    }
}

function collectMonitorInfo(cell, monitorInfoSet) {
    if (cell == null) {
        return;
    }
    if (cell.gettype() == "title") {
    ...
    if (cell.isVertex() && cell.getattr() == "property_data") {
        var monitor_device_name = cell.getmonitor_device_name();
        console.log("**********************************");
        console.log("monitor_device_name：  " + cell.getmonitor_device_name());
        console.log("monitor_name：  " + cell.getname());
        console.log("monitor_unit：  " + cell.getmonitor_unit());
        console.log("**********************************");

        var monitorInfo = cell.getmonitor_device_name() + ":" + cell.getname();
        monitorInfoSet.add(monitorInfo);
    }
    for (var i = 0; i < cell.getChildCount(); i++) {
        var child = cell.getChildAt(i);
        collectMonitorInfo(child, monitorInfoSet);
    }
}

function hasmonitor(monitor_name, jsonArray) {
    for (var i = 0; i < jsonArray.length; i++) {
        if (jsonArray[i]["data_name"] == monitor_name) {
            var value = jsonArray[i]["detected_value"];
            var alarm_state = jsonArray[i]["alarm_state"];
            return value + ":" + alarm_state;
        }
    }
    return null;
}

function updateCellValue(cell, jsonData) { //递归调用
    if (cell == null || jsonData == null) {
        return;
    }
    ...
    if (cell.isVertex() && cell.getattr() == "property_data") {
        let monitor_device_name = cell.getmonitor_device_name();
        let monitor_name = cell.getname();

        if (hasmonitor(monitor_name, jsonArray) != null) {
            let res = hasmonitor(monitor_name, jsonArray);
            console.log("res：  " + res);
            let value = res.split(":")[0];
            let alarm_state = res.split(":")[1];
            if (alarm_state == 1) {
                //报警，字体变红
                console.log("报警.... ");
                var new_style = "fillColor=#FFFFFF;gradientColor=#FFFFFF;fontFamily=微软雅黑;fontSize=14;fontColor=#FF3030;strokeColor=#000000;strokeWidth=1;labelPosition=center;align=center";
                cell.setstyle(new_style);
                cell.setValue(value);
                //当需要报警的时候，先判断alarmCells是否有报警图元
                //如果有，设为可见
                //没有，创建一个新的，添加到alarmCells中
                var isnew=true; //假设需要new一个新的alarmCell
                alarmCells.forEach(function (element, index, array) {
                    // element: 指向当前元素的值
                    // index: 指向当前索引
                    // array: 指向Array对象本身
                    if(monitor_name==element.getname()){
                        isnew=false;
                ...
                if(isnew){
                    let alarmCell = new mxCell();
                    let x=cell.getGeometry().x;
                    let y=cell.getGeometry().y;
                    alarmCell.setGeometry(new mxGeometry(x+110, y, 32, 32));
                    alarmCell.setStyle(alarmImg);
                    alarmCell.setname(monitor_name);
                    alarmCell.settype("Alarm");
                    alarmCell.setattr("alarm");
                    alarmCell.setVertex(true);
                    alarmCell.setConnectable(false);
                    alarmCell.setVisible(true);
                    mGraph.getDefaultParent().insert(alarmCell);
                    alarmCells.add(alarmCell);
                }
            }
            if (alarm_state == 0) {
                var old_style = "fillColor=#FFFFFF;gradientColor=#FFFFFF;fontFamily=微软雅黑;fontSize=14;fontColor=#000099;strokeColor=#000000;strokeWidth=1;labelPosition=center;align=center";
                cell.setstyle(old_style);
                cell.setValue(value);
    ...
    for (var i = 0; i < cell.getChildCount(); i++) {
        var child = cell.getChildAt(i);
        updateCellValue(child, jsonData);
    }
}
...
function requestCallBack(xmlHttp) {
    if (xmlHttp.readyState == 4 && xmlHttp.status == 200) { // finish
        ...
        updateCellValue(mRootCell, JSON.stringify(jsonData));
        mGraph.refresh();
    }
}

function requestCellValue(monitorInfoSet) {
    var resStr = "";
    monitorInfoSet.forEach(function (element, index, array) {
        // element: 指向当前元素的值
        // index: 指向当前索引
        // array: 指向Array对象本身
    ...
    console.log("monitorInfoSet=" + resStr);
    //请求数据服务  Access-Control-Allow-Origin:
    var ip = "http://localhost";
    var url = ip + ":8888/telemetry/findMonitorValue/" + site_name + "/" + resStr;
    console.log("url:  " + url);
...
function startService() {
    ...
    console.log("startService - updating");
    var monitorInfoSet = new Set();
    //collectDeviceId(mRootCell, deviceIdSet);
    collectMonitorInfo(mRootCell, monitorInfoSet);
    console.log("-----------------------");
    requestCellValue(monitorInfoSet);
    // update multi-cell style
    updateCellStyle(mRootCell);
    mGraph.refresh();
    //1s刷新一次
...
function updateCellStyle(cell) {
    ...
    //暂时没有用到，没有多值图元
    if (cell.isVertex() && cell.getmultiValue() == "1") {
        doUpdateCellStyle(cell);
    }
    for (var i = 0; i < cell.getChildCount(); i++) {
        var child = cell.getChildAt(i);
...
function doUpdateCellStyle(cell) {
    if (cell == null || cell.getOriginStyle() == null) {
        return;
    }
    var styles = cell.getOriginStyle().split("|");
    if (styles == null) {
        return;
    }
    var idx = 0;
    for (var i = 0; i < styles.length; ++i) {
        if (cell.getStyle() == styles[i]) {
            idx = i;
            break;
        }
    }
    idx = (idx + 1) % styles.length;
    cell.setStyle(styles[idx]);
    console.log("update style: " + cell.getStyle());
}

function sendCloseCmd(infoStr) {
    var isSucceed = "false";
    //请求数据服务  Access-Control-Allow-Origin:
    var ip = "http://localhost";
    var url = ip + ":8888/wsn/sendCloseCmdById/" + infoStr;
    console.log("url:  " + url);
    var xmlHttp = createXMLHttpRequest();
    //xmlHttp.crossDomain="Access-Control-Allow-Origin";
    if (xmlHttp == null) {
...
function openGraph(graphxml,sitename) {
    var ui1=editorui1;
    var editor1=editorui1.editor;
    ...
    try
    {
        ...
        editor1.undoManager.clear();

        if (sitename != null)
        {
            editor1.setFilename(sitename);
            ui1.updateDocumentTitle();
        }
        return;
    }
    catch (e)
...
Actions.prototype.init = function () {
    ...
    //王伟测试TEST_WW业务
    this.addAction("Test_WW", function () {
        WW.showMiniDialog({
            title: "提交成功！",
            content:"修改检测值成功！",
            type:"warningx",
    ...
    this.addAction("EditDetectedValue", function () {
        ...
        let params = {
            hasProperty: selectedCell.gethasproperty(),
            deviceId: selectedCell.getdeviceid(),
            mRootCell: graph.model.root,
            site_name: site_name,
            graph:graph
        }
        WW.showEditDetectedValueDialog(params);
    });
...
/*    this.put('opener', new Action('opener', function () {
            ...
            var xml="<mxGraphModel><root><MyDiagram id=\"0\"><mxCell/></MyDiagram>" +
    ...
    this.addAction('sendCommand', function () {
